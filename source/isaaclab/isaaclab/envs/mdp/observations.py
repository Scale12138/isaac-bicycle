...
def base_ang_vel(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    return asset.data.root_ang_vel_b[:, 0].unsqueeze(-1) / torch.pi * 180.0
...
def joint_pos(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    # print('joint_pos: ', asset.data.joint_pos[:, :].unsqueeze(-1).detach().cpu().numpy())
    return asset.data.joint_pos[:, 0].unsqueeze(-1) / torch.pi * 180.0
...
def joint_vel(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
    ...
    return asset.data.joint_vel[:, 1].unsqueeze(-1)


def joint_vel_target(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
    """返回当前关节速度与目标速度的差值"""
    asset: Articulation = env.scene[asset_cfg.name]
    joint_vel = asset.data.joint_vel[:, 1]
    joint_vel_target = torch.full((env.num_envs, 1), -3.0, device='cuda').squeeze(-1)
    return torch.abs(joint_vel - joint_vel_target)
